    ...
    <div className="flex flex-col gap-6">
      ...
      {topClients.length > 0 && (
        <div className="rounded-lg border border-black/10 bg-white p-4 dark:border-white/10 dark:bg-zinc-950">
          <p className="mb-2 text-xs text-zinc-500">Opportunities by client</p>
          <div className="flex flex-wrap gap-2">
            {topClients.map(([name, count]) => (
              <span key={name} className="rounded-full bg-zinc-100 px-3 py-1 text-xs dark:bg-zinc-800">
                {name} · {count}
              </span>
            ))}
          </div>
      ...
      <div className="overflow-hidden rounded-lg border border-black/10 dark:border-white/10">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-black/5 align-top dark:border-white/5">
                ...
                <td className="px-4 py-2 whitespace-nowrap text-zinc-500">{formatMalaysiaDate(r.createdAt)}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr>
